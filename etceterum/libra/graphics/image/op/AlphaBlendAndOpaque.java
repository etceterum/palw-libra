...
// (Rtgt, Gtgt, Btgt) = (Rsrc, Gsrc, Bsrc)*Asrc + (Rtgt, Gtgt, Btgt)*(1 - Asrc),
// Atgt = 1 (opaque)
public final class AlphaBlendAndOpaque extends SourceOp {
    
    public AlphaBlendAndOpaque() {
        // no-op
    }
    
    public AlphaBlendAndOpaque(Image source) {
        super(source);
    }
    ...
    public Image execute(Image input) {
        ...
        if (sourceWidth == targetWidth && sourceHeight == targetHeight) {
            int sourceR, sourceG, sourceB, sourceA, targetR, targetG, targetB, oneMinusSourceA, r, g, b;
            ...
            for (int y = 0; y < sourceHeight; ++y, sourceIdx += sourceStrideInBytes, targetIdx += targetStrideInBytes) {
                ...
                for (int x = 0; x < sourceWidth; ++x, sourcePixelIdx += 4, targetPixelIdx += 4) {
                    ...
                    targetData[3 + targetPixelIdx] = (byte)0xFF;
